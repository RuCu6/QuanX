...
function ResourceParse() {
  ...
  if (type0 == "Subs-B64Encode") {
    ...
    $notify("‼️ 引用" + "⟦" + subtag + "⟧" + " 返回内容为空", "⁉️ 点通知跳转以确认链接是否失效", para.split("#")[0], nan_link);
    flag = 0;
...
function SubFlow() {
  if (Pinfo == 1 && subinfo) {
    ...
    if (sinfo.indexOf("expire=") != -1) {
      ...
      epr = ""; // "过期时间: ✈️ 未提供该信息" // 没过期时间的显示订阅链接
    }
    var message = total + "\n" + usd + ", " + left;
    ntf_flow = 1;
...
function Type_Check(subs) {
  ...
  } else if ((((ModuleK.some(RewriteCheck) || para1.indexOf("dst=rewrite") != -1) && para1.indexOf("dst=filter") == -1 && subs.indexOf("[Proxy]") == -1) || typeU == "module") && typeU != "nodes" && typeU != "rule" && typeQ != "filter") {
...
function Fobfs(jsonl, Pcert0, PTls13) {
  ...
  if (jsonl.net == "ws" && jsonl.tls == "tls") {
    ...
    obfsi.push(obfs0, host0 + uri0);
    return obfsi.join(", ");
  } else if (jsonl.net != "tcp" && jsonl.net != "none" && jsonl.net != undefined) {
    // 过滤掉 h2/http 等类型
    Perror = 1;
...
function VL2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
  ...
  } else if (cnt.indexOf("&type=ws") != -1 || cnt.indexOf("?type=ws") != -1 || cnt.indexOf("type=http") != -1 || cnt.indexOf("security=tls") != -1) {
    // v2rayN uri
  ...
  function preProcess(src) {
    ...
    for (var i in lines) {
      if ((m = lines[i].match(r))) {
        // var cmt = "";
        // if (typeof m[3] != "undefined") lines[i] = m[1];
        // else if (typeof m[3] != "undefined") lines[i] = m[3];
        // else lines[i] = "";
